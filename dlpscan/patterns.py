...
# DLP Scan Patterns
#
# Each top-level key is a category. Each value is a dict mapping a sub-category
# name to a compiled regex pattern.  All patterns use word boundaries (\b) or
# look-arounds where appropriate to avoid partial matches.
# =============================================================================
...
PATTERNS = {
    ...
    # =========================================================================
    'Personal Identification': {
        # Canada Social Insurance Number: XXX-XXX-XXX or XXXXXXXXX
        'Canada SIN': re.compile(r'\b\d{3}[-\s]?\d{3}[-\s]?\d{3}\b'),

        # USA Social Security Number: XXX-XX-XXXX or XXXXXXXXX
        'USA SSN': re.compile(r'\b\d{3}[-\s]?\d{2}[-\s]?\d{4}\b'),

        # USA Individual Taxpayer Identification Number: 9XX-XX-XXXX (starts with 9)
        'USA ITIN': re.compile(r'\b9\d{2}[-\s]?\d{2}[-\s]?\d{4}\b'),

        # USA Employer Identification Number: XX-XXXXXXX
        'USA EIN': re.compile(r'\b\d{2}-\d{7}\b'),

        # UK National Insurance Number: XX999999X (restricted first-letter pairs, last letter A-D)
        'UK NIN': re.compile(r'\b[A-CEGHJ-PR-TW-Z]{2}\d{6}[A-D]\b'),

        # UK Unique Taxpayer Reference: 10 digits
        'UK UTR': re.compile(r'\b\d{10}\b'),

        # Singapore NRIC: [STFGM]XXXXXXX[A-Z]
        'Singapore NIRC': re.compile(r'\b[STFGM]\d{7}[A-Z]\b'),

        # Australia Tax File Number: 8 or 9 digits
        'Australia TFN': re.compile(r'\b\d{8,9}\b'),

        # India PAN: XXXXX9999X (5 letters, 4 digits, 1 letter)
        'India PAN': re.compile(r'\b[A-Z]{5}\d{4}[A-Z]\b'),

        # India Aadhaar Number: 12 digits with optional spaces/hyphens
        'India Aadhaar': re.compile(r'\b\d{4}[\s-]?\d{4}[\s-]?\d{4}\b'),

        # Germany ID: 9-character alphanumeric (new format, excludes I/O/S)
        'Germany ID': re.compile(r'\b[CFGHJKLMNPRTVWXYZ0-9]{9}\b'),

        # France INSEE/NIR (Social Security): 13 digits + 2-digit key
        ...
        # Netherlands BSN (Burgerservicenummer): 8 or 9 digits
        ...
        # Japan My Number (Individual Number): 12 digits
    ...
    'Health Cards': {
        ...
        # US DEA Number: 2 letters + 7 digits (prescriber registration)
        'US DEA Number': re.compile(r'\b[A-Z]{2}\d{7}\b'),

        # US NPI (National Provider Identifier): 10 digits, starts with 1 or 2
    ...
    'Bank Account Numbers': {
        ...
        # USA routing number: 9 digits
        'USA Routing Number': re.compile(r'\b\d{9}\b'),

        # Canada transit number: XXXXX-XXX
    ...
    # =========================================================================
    # DATES (potential DOB / sensitive dates when near context keywords)
    # =========================================================================
    'Dates': {
