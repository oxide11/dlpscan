...
PATTERNS = {
    'Personal Identification': {
        # Canadian Social Insurance Number pattern with optional spaces or hyphens between the groups
        # (formatted as XXX-XXX-XXX or XXXXXXXXX)
        'Canada SIN': re.compile(r'\b\d{3}[-\s]?\d{3}[-\s]?\d{3}\b'),
        
        # United States Social Security Number pattern with optional spaces or hyphens between the groups
        # (formatted as XXX-XX-XXXX or XXXXXXXXX)
        ...
        # Spain DNI/NIE pattern allowing an optional leading character (X/Y/Z) followed by either 7 or 8 digits
        # and finally a letter
        'Spain DNI/NIE': re.compile(r'\b[XYZ]?\d{7,8}[A-Z]\b'),
    },
    'Credit Card Numbers': {
        # Visa card pattern with optional spaces or hyphens between 4-digit groups
        # (formatted as XXXX XXXX XXXX XXXX)
        'Visa': re.compile(r'4\d{3}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{4}'),
        
        # MasterCard pattern with optional spaces or hyphens between 4-digit groups
        # (formatted as 5XXX XXXX XXXX XXXX where X can be 1-5)
        'MasterCard': re.compile(r'5[1-5]\d{2}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{4}'),
        
        # American Express card pattern with optional spaces or hyphens after 4 and 6 digits
        # (formatted as 34XX XXXXXX XXXXX or 37XX XXXXXX XXXXX)
        'Amex': re.compile(r'3[47]\d{2}[\s-]?\d{6}[\s-]?\d{5}'),
        
        # Discover card pattern with optional spaces or hyphens between 4-digit groups
        # (formatted as 6011 XXXX XXXX XXXX or 65XX XXXX XXXX XXXX)
    ...
    'Driver Licenses': {
        ...
        # Patterns for Alberta, Quebec, and Nova Scotia follow similar structures to those above,
        # adjusted for their specific formatting as required by each province.
        'Alberta DL': re.compile(r'\b[A-Z]\d{4,9}\b'),             # Alberta format example
        
        'Quebec DL': re.compile(r'\b[A-Z]{2}\d{4,9}\b'),           # Quebec format example
        
        'Nova Scotia DL': re.compile(r'\b[A-Z]{2}\d{4,9}\b'),      # Nova Scotia format example
    },
    'Health Cards': {
        ...
        # Alberta health card pattern with specific grouping, formatted as X9999-99999
        'Alberta HC': re.compile(r'\b[A-Z]\d{4}-\d{5}\b'),
        
        # Quebec health card pattern with 12 consecutive digits
        'Quebec HC': re.compile(r'\b\d{12}\b'),
        
        # Nova Scotia health card pattern with 10 consecutive digits
    ...
    'Bank Account Numbers': {
        # Generic IBAN pattern with proper grouping and optional spaces, starting with two letters followed by
        # two digits, then up to 31 alphanumeric characters in groups of four
        ...
        # SWIFT/BIC pattern with standard format including 4-letter bank code, 2-letter country code,
        # 2-alphanumeric location code, and an optional 3-character branch code
