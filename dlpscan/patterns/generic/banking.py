...
BANKING_PATTERNS = {
    ...
    'Banking and Financial': {
        ...
        # US Bank Account Number — require context-only (too broad alone)
    ...
    'Wire Transfer Data': {
        ...
        # CHIPS UID (6-digit participant + sequence, structured format)
        'CHIPS UID': re.compile(r'\b\d{6}[A-Z0-9]{4,10}\b'),
        # Wire Reference Number (16-35 alphanumeric, must mix letters and digits)
        'Wire Reference Number': re.compile(r'\b(?=[A-Z0-9]*[A-Z])(?=[A-Z0-9]*\d)[A-Z0-9]{16,35}\b'),
        # ACH Trace Number (15 digits: 8 routing + 7 sequence)
        'ACH Trace Number': re.compile(r'\b(?:0[1-9]|[12]\d|3[0-2]|6[1-9]|7[0-2])\d{13}\b'),
        # ACH Batch Number (7 digits — context-gated, too short alone)
        'ACH Batch Number': re.compile(r'\b\d{7}\b'),
        # SEPA End-to-End Reference (12-35 chars, must mix letters and digits)
        'SEPA Reference': re.compile(r'\b(?=[A-Z0-9]*[A-Z])(?=[A-Z0-9]*\d)[A-Z0-9]{12,35}\b'),
    },
    # -- Check/Cheque Data ------------------------------------------------
    'Check and MICR Data': {
        # MICR Line (routing + account + check number with MICR symbols)
        'MICR Line': re.compile(r'[⑈❰]?\d{9}[⑈❰]?\s?\d{6,17}[⑈❰]?\s?\d{4,6}'),
        # Check Number (4-6 digits — context-gated)
        'Check Number': re.compile(r'\b\d{4,6}\b'),
        # Cashier Check / Money Order Number (8-15 digits)
    ...
    'Securities Identifiers': {
        ...
        # Ticker Symbol (1-5 uppercase letters — require leading $ or context)
        'Ticker Symbol': re.compile(r'(?<!\w)\$[A-Z]{1,5}\b'),
    },
    # -- Loan & Mortgage --------------------------------------------------
    'Loan and Mortgage Data': {
        # Loan Number (8-15 alphanumeric, must mix letters and digits)
        'Loan Number': re.compile(r'\b(?=[A-Z0-9]*[A-Z])(?=[A-Z0-9]*\d)[A-Z0-9]{8,15}\b'),
        ...
        'Universal Loan Identifier': re.compile(r'\b[A-Z0-9]{4}00[A-Z0-9]{17,39}\b'),
        # Loan-to-Value ratio (##.##%)
        'LTV Ratio': re.compile(r'\b\d{1,3}\.\d{1,2}%\b'),
    },
    # -- Regulatory & Compliance ------------------------------------------
    'Regulatory Identifiers': {
        # SAR (Suspicious Activity Report) Filing Number (14-20 digits)
        'SAR Filing Number': re.compile(r'\b\d{14,20}\b'),
        # CTR (Currency Transaction Report) Number (14-20 digits)
        'CTR Number': re.compile(r'\b\d{14,20}\b'),
        # BSA/AML Case ID (prefix + digits)
        'AML Case ID': re.compile(r'\b[A-Z]{2,4}[-]?\d{6,12}\b'),
        # OFAC SDN List Entry ID (4-6 digits — context-gated)
        'OFAC SDN Entry': re.compile(r'\b\d{4,6}\b'),
        # FinCEN Report Number (14 digits)
    ...
    'Internal Banking References': {
        # Internal Account Reference (prefix letters + digits)
        'Internal Account Ref': re.compile(r'\b[A-Z]{2,4}\d{8,14}\b'),
        # Teller/Officer ID (prefix + digits)
        'Teller ID': re.compile(r'\b[A-Z]{1,3}\d{4,8}\b'),
    },
    # -- Payment Card Industry (PCI) --------------------------------------
    'PCI Sensitive Data': {
        # Cardholder Name (First Last — context-gated)
