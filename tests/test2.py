from dlpscan.scanner import enhanced_scan_text, redact_sensitive_info, redact_sensitive_info_with_patterns
...
test_cases = [
    ...
    # (Text, Description, Expects Pattern Match, Expects Context Match)
      ("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla at nunc eu dui suscipit dignissim. Pellentesque aliquam elit nec tortor faucibus, et pharetra augue porttitor. Sed lobortis, mauris vel commodo imperdiet, odio leo porta urna, quis semper eros arcu eget mi. Nam lorem tellus, maximus nec faucibus at, placerat nec erat. Fusce et sollicitudin turpis. Cras vehicula mauris ligula, at malesuada lorem porttitor at. Phasellus id augue gravida, dignissim nulla nec, dapibus orci. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia curae; Morbi sollicitudin nunc non sollicitudin molestie. Pellentesque ullamcorper orci non nisi imperdiet, quis imperdiet dolor laoreet. In auctor id lectus vitae bibendum. Sed ultrices sapien urna, id porttitor ligula dapibus id. Proin tempus dignissim augue posuere elementum. My SIN number is 123-456-989 ", "Canada SIN", True, False),
...
def run_tests():
    for text, description, expects_pattern, expects_context in test_cases:
        ...
        findings = list(enhanced_scan_text(text))
        ...
        if pattern_detected:
            ...
            for finding in findings:
                matched_pattern, label, has_context, category, sub_category = finding
                print(f"Matched: {matched_pattern} | Label: {label} | Context Detected: {has_context} | Category: {category} | Sub-Category: {sub_category}")
                redacted_text = redact_sensitive_info_with_patterns(text, category, sub_category)
